...
public class BestFirstSearcher<T extends BestFirstObject<T>> {
    ...
    private class SearchNode implements Comparable<SearchNode> {
        ...
        public SearchNode(SearchNode parent, T pNode, T pGoal) {
            node = pNode;
            //goal = pGoal;
            this.parent = parent;
            depth = (parent == null) ? 0 : parent.depth + 1;
            dist = h.getDistance(pNode, pGoal);
            if (debug) {System.out.println(getObject() + "; " + pGoal + ": " + value());}
        }
