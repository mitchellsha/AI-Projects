...
public class Perceptron extends PerceptronNet {
    ...
    public double[] compute(double[] inputs) {
        ...
        for (int i = 0; i < numOutputNodes(); ++i) {
            ...
            //Debug("Output[" + i + "]: " + outputs[i]);
            
            total -= weights[threshold()][i];
            outputs[i] = sigmoid(total);
            //outputs[i] = total - sig;
        }
        return outputs;
    }
    ...
    public void addToWeightDeltas(double[] inputs, double rate) {
        ...
        for (int i = 0; i < numOutputNodes(); ++i) {
    		double weight = errors[i] * rate * gradient(outputs[i]);
            for (int j = 0; j < numInputNodes(); ++j) {
        		deltas[j][i] += weight * inputs[j];
            }
            
            deltas[threshold()][i] += weight * -1;
        }
    }
