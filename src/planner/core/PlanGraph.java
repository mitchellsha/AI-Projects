...
public class PlanGraph {
	...
	private final static boolean MYDEBUG = false;
	...
	public Plan extractNoDeletePlan(State current) {
		...
		if (MYDEBUG) {System.out.println("current: " + current);}

		ArrayList<Predicate> predicates = new ArrayList<Predicate>();
		for(Predicate g: goals){
			if (MYDEBUG) {System.out.println("goal: " + g);}
			predicates.add(g);
		}
		
		ArrayList<Action> plan = new ArrayList<Action>();
		while(!predicates.isEmpty()){
			Predicate pred = predicates.remove(0);
			if (MYDEBUG) {System.out.println("predicate: " + pred + "\t" + pred.isTrue());}
			if(!current.predIsTrue(pred)){
				Action firstAction = firstAdders.get(pred);
				plan.add(0, firstAction);
				State preconds = firstAction.getPreconditions();
		...
		NoDeletePlan ndp = new NoDeletePlan();
		for(Action a: plan){
			if(!ndp.contains(a)){
				if (MYDEBUG) {System.out.println("action: " + a);}
				ndp.appendAction(a);
			}
		}

		if (MYDEBUG) {System.out.println("result: " + ndp + "\n");}
		return ndp;
	}
